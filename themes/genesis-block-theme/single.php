<?php
/**
 * The Template for displaying all single posts.
 *
 * @package Genesis Block Theme
 */

get_header();

$header_class= "";

if ( has_post_thumbnail() ) {
	$header_class = 'post__has-thumbnail';
}
?>
<article class="post__single <?php echo $header_class; ?>">
	<header
		class="wp-block-group page__header has-purple-background-color has-background is-layout-constrained wp-container-core-group-is-layout-1 wp-block-group-is-layout-constrained">
		<div class="post__back">
			<a href="<?php echo get_home_url(); ?>/blog">All posts</a>
		</div>
		<h1><?php the_title(); ?></h1>
	</header>
	<main class="post__content">
		<figure class="post__thumbnail">
			<?php echo get_the_post_thumbnail( get_the_ID(), 'full' ); ?>
		</figure>

		<?php the_content(); ?>

		<div class="post__share">
			<a href="" class="share__icon">
				<svg width="44" height="44" viewBox="0 0 44 44" fill="none" xmlns="http://www.w3.org/2000/svg">
					<rect width="44" height="44" rx="22" fill="#8B3B70" />
					<path
						d="M28 12H25C23.6739 12 22.4021 12.5268 21.4645 13.4645C20.5268 14.4021 20 15.6739 20 17V20H17V24H20V32H24V24H27L28 20H24V17C24 16.7348 24.1054 16.4804 24.2929 16.2929C24.4804 16.1054 24.7348 16 25 16H28V12Z"
						fill="white" />
				</svg>
			</a>
			<a href="" class="share__icon">
				<svg width="44" height="44" viewBox="0 0 44 44" fill="none" xmlns="http://www.w3.org/2000/svg">
					<rect width="44" height="44" rx="22" fill="#8B3B70" />
					<path
						d="M26 18C27.5913 18 29.1174 18.6321 30.2426 19.7574C31.3679 20.8826 32 22.4087 32 24V31H28V24C28 23.4696 27.7893 22.9609 27.4142 22.5858C27.0391 22.2107 26.5304 22 26 22C25.4696 22 24.9609 22.2107 24.5858 22.5858C24.2107 22.9609 24 23.4696 24 24V31H20V24C20 22.4087 20.6321 20.8826 21.7574 19.7574C22.8826 18.6321 24.4087 18 26 18Z"
						fill="white" />
					<path d="M16 19H12V31H16V19Z" fill="white" />
					<path
						d="M14 16C15.1046 16 16 15.1046 16 14C16 12.8954 15.1046 12 14 12C12.8954 12 12 12.8954 12 14C12 15.1046 12.8954 16 14 16Z"
						fill="white" />
				</svg>
			</a>
			<a href="" class="share__icon">
				<svg width="44" height="44" viewBox="0 0 44 44" fill="none" xmlns="http://www.w3.org/2000/svg">
					<rect width="44" height="44" rx="22" fill="#8B3B70" />
					<path
						d="M23.9047 20.4696L31.3513 12H29.5873L23.1187 19.3525L17.956 12H12L19.8087 23.1194L12 31.9999H13.764L20.5907 24.2338L26.044 31.9999H32M14.4007 13.3016H17.1107L29.586 30.7623H26.8753"
						fill="white" />
				</svg>
			</a>
			<a href="" class="share__icon">
				<svg width="44" height="44" viewBox="0 0 44 44" fill="none" xmlns="http://www.w3.org/2000/svg">
					<rect width="44" height="44" rx="22" fill="#8B3B70" />
					<path
						d="M14 22V30C14 30.5304 14.2107 31.0391 14.5858 31.4142C14.9609 31.7893 15.4696 32 16 32H28C28.5304 32 29.0391 31.7893 29.4142 31.4142C29.7893 31.0391 30 30.5304 30 30V22M26 16L22 12M22 12L18 16M22 12V25"
						stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
				</svg>
			</a>
		</div>
	</main>
</article>

<aside class="post__related">
	<header>
		<h3>Recent Posts</h3>
	</header>

	<?php
			$template = file_get_contents( get_template_directory(  ) . '/templates/related-posts.html' );

			echo do_blocks( $template );
		?>
</aside>

<?php 
	$pattern_id = 309;
	$content_pattern = get_post($pattern_id);        
	$content = $content_pattern->post_content;

	echo do_blocks( $content );
?>

<?php get_footer(); ?>