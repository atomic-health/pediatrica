...
function event_get_date( $post_id ) {
  $start = get_post_meta( $post_id, '_EventStartDate' );
...
add_filter( 'meta_field_block_get_block_content', function ( $block_content, $attributes, $block, $object_id, $object_type ) {
  ...
  if ( 'event_date' === $field_name ) {
    $block_content = event_get_date( $object_id );
  }
 ...
 */
function event_get_time( $post_id ) {
  $all_day = get_post_meta( $post_id, '_EventAllDay' );
  $start = get_post_meta( $post_id, '_EventStartDate' );
  $end = get_post_meta( $post_id, '_EventEndDate' );
...
add_filter( 'meta_field_block_get_block_content', function ( $block_content, $attributes, $block, $object_id, $object_type ) {
  ...
  if ( 'event_time' === $field_name ) {
    $block_content = event_get_time( $object_id );
  }
 ...
 */
function event_get_location( $post_id ) {
  $block_content = '';
  $location_id = get_post_meta( $post_id, '_EventVenueID' );
...
add_filter( 'meta_field_block_get_block_content', function ( $block_content, $attributes, $block, $object_id, $object_type ) {
  ...
  if ( 'event_location' === $field_name ) {
    $block_content = event_get_location( $object_id );
  }
